...
def run_assistant(
    ...
    while True:
        ...
        stream = retry_with_backoff(_make_call)
        ...
        turns += 1
        cost_dict = estimate_cost(user_msg, full_reply, model=OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        prompt_str = str(result.get("prompt", ""))
        gpt4o_ans = str(result.get("gpt4o_answer") or result.get("gpt4o_response", ""))
        mini_ans = str(result.get("mini_answer") or result.get("mini_response", ""))
        gpt4o_lat = float(result.get("gpt4o_time", result.get("gpt4o_latency", 0.0)))
        mini_lat = float(result.get("mini_time", result.get("mini_latency", 0.0)))
